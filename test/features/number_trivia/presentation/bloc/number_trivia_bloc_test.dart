...
void main() {
  ...
  group('GetTriviaForConcreteNumber', () {
    ...
    void setUpMockInputConverterSuccess() => when(mockInputConverter.stringToUnsignedInterger(any)).thenReturn(Right(tNumberParsed));
    ...
    test(
      ...
      () async {
        ...
        await untilCalled(mockInputConverter.stringToUnsignedInterger(any)); //on doit attendre un peu car parfois bloc mets un peu de temps, fonction de mockito le await untilCalled
        //assert
    ...
    test(
      ...
      () async {
        //arrange
        when(mockInputConverter.stringToUnsignedInterger(any)).thenReturn(Left(InvalidInputFailure()));
        //assert later - ici on met assert avant act pour des raisons de sureté, s'assurer que ce qu'on attend est bien défini avant l'execution
        final expected =  [Empty(), Error(message: INPUT_FAILURE_MESSAGE),];
        expectLater(bloc.state, emitsInOrder(expected)); //expectLater car on travail ici avec des streams
        //act
        bloc.dispatch(GetTriviaForConcreteNumber(tNumberString));
      },
    );
